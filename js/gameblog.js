(function () {
    ...
    function getGameBlogPosts() {
        return Array.isArray(window.GAMEBLOG_POSTS_DATA) ? window.GAMEBLOG_POSTS_DATA.slice() : [];
    }
    ...
    function renderFilters(posts) {
        var wrap = document.getElementById('gameblog-filters');
        if (!wrap) return;
        var lang = getLang();
        var current = wrap.querySelector('.active')?.dataset.category || 'all';
        var labels = { all: lang === 'en' ? 'All' : '全部' };
        var html = '<button type="button" class="topic-filter' + (current === 'all' ? ' active' : '') + '" data-category="all">' + labels.all + '</button>';
    ...
    function renderCards(posts) {
        var grid = document.getElementById('gameblog-grid');
        if (!grid) return;
        var lang = getLang();
        if (!posts.length) {
            grid.innerHTML = '<div class="empty-topic">' + (lang === 'en' ? 'No game posts found.' : '暂无匹配的游戏文章。') + '</div>';
            return;
        }
        ...
        grid.innerHTML = posts.map(function (post) {
            ...
            return [
                ...
                '<a class="topic-read" href="' + post.url + '">' + (lang === 'en' ? 'Read post' : '阅读文章') + '</a>',
                '</div>',
                '</article>'
            ].join('');
        }).join('');
    ...
    function updateStats(posts) {
        var count = document.getElementById('gameblog-count');
        if (count) count.textContent = posts.length;
    }

    function renderGameBlog() {
        var posts = getGameBlogPosts().sort(function (a, b) {
            return Date.parse(b.date) - Date.parse(a.date);
        });
        renderFilters(posts);

        var active = document.querySelector('#gameblog-filters .active')?.dataset.category || 'all';
        var term = (document.getElementById('gameblog-search')?.value || '').trim().toLowerCase();
    ...
    window.getGameBlogPosts = getGameBlogPosts;
    window.renderGameBlog = renderGameBlog;

    document.addEventListener('DOMContentLoaded', function () {
        var search = document.getElementById('gameblog-search');
        if (search) search.addEventListener('input', renderGameBlog);
        renderGameBlog();
    });

    window.addEventListener('sitewide-language-change', renderGameBlog);
})();
